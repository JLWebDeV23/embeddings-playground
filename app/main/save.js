<>
    ...
    <button
        className="btn btn-square ml-4"
        onClick={() => {
            const modal = document.getElementById("string_interpolation_modal");
            if (modal) {
                modal.showModal();
                setIsModalOpen(true);
            }
    ...
    <dialog id="string_interpolation_modal" className="modal">
        <div className="modal-box">
            ...
            <h3 className="font-bold text-lg pb-2">String /</h3>
            {stringInterpolationList &&
                stringInterpolationList.map((list, index) => (
                    <div
                        className="flex border border-slate-600 border-b-0"
                        key={index}
                    >
