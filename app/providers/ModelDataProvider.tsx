...
*/
      ...
      case 'add':
        ...
        setApiModelData(
          modelData.map((interpolationAnswer) => [
            ...
            {
              ...newModelData(model),
              messages: modelData[0][0].messages.map((message) => {
                if (message.role === 'user' || message.role === 'system') {
                  return message;
                }
                return {
                  ...message,
                  content: message.content.replaceAll(/./g, () =>
  ...
  const [interpolations, setInterpolations] = useState<StringInterpolations[]>([
    {
      list: [
        {
          key: 0,
        ...
        const list = page.list.map((prev, j) => {
          if (j === index) {
            return {
              ...
              field: field && pageNumber === i ? field : prev.field,
            };
          }
          return prev;
        });
