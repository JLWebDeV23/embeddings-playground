              ...
              <ModalBody>
                <Tabs
                  ...
                  onSelectionChange={(key) => {
                    ...
                    if (key === interpolations.length.toString()) {
                      changeInterpolationVariables({
                        pageNumber: interpolations.length,
                      });
                    }
                  }}
                >
                  {interpolations?.map((interpolation, page) => (
                    <Tab key={page} title={`Page ${page + 1}`}>
                      <InterpolationPage
                        onDeletePage={() => {
                          setSelectedPage(
                            page - 1 > 0 ? (page - 1).toString() : '0'
                          );
                        }}
                  ...
                  <Tab title="New Page" key={interpolations.length}></Tab>
                </Tabs>
              </ModalBody>
              <ModalFooter>
...
function InterpolationInput({
  ...
  return (
    <>
      <div className="flex gap-3 items-center" key={index}>
        ...
        }
        {
          ...
          index === interpolations.list.length && (
            <Button
              color="primary"
              variant={field === '' || variable === '' ? 'light' : 'flat'}
      ...
      {(field !== '' || variable !== '') &&
        index === interpolations.list.length && (
          <p className="text-warning/70 text-xs -mb-7">New Entry not saved</p>
        )}
    </>
  );
}
