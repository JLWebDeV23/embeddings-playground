...
const StringInterpolationDisplay: React.FC<StringInterpolationDisplayProps> = ({
  ...
  useEffect(() => {
    setFieldValues(interpolation.map((item) => item.field || ''));
  }, [interpolation]);
  ...
  const handleOnDeleteBtn = (index: number) => {
    const updatedInterpolations = stringInterpolations.map((interpolation) => {
      const updatedList = interpolation.list.filter((_, i) => i !== index);
      return { list: updatedList };
    });
    setStringInterpolations(updatedInterpolations);
    const updatedFieldValues = fieldValues.filter((_, i) => i !== index);
